/**
 * @file navigation_refactored_node.cpp 
 * @brief 导航功能包最外层
 * 
 * 支持差速底盘和四转四驱底盘
 * 
 ...
 *   - 用于改善控制平滑性
 *   - 无里程计时依然可以进行导航
 * 20260129 将原导航的大部分功能移植到了现在的代码中，手动暂停和停止的功能要继续区分导航和非导航情况  以及
...
public:
    NavigationRefactoredNode() : Node("navigation_example_node") {
        ...
        // 声明参数 - 话题名称
        this->declare_parameter("slam_pose_topic", STATE_POS);   // 激光SLAM位姿（必须）
        this->declare_parameter("odom_topic", STATE_ODOM);             // 里程计（可选，用于速度反馈）
        ...
        LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "Subscribing to SLAM pose: %s (REQUIRED)", slam_pose_topic.c_str());
        ...
        if (use_odom_feedback_) {
            ...
            LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "Subscribing to odometry: %s (OPTIONAL, for velocity feedback)", odom_topic.c_str());
        } else {
            LOG4CPLUS_WARN_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "Odometry feedback disabled, using open-loop control");
        }

        //IMU订阅，暂时没用到
        m_Imu_sub_ = this->create_subscription<sensor_msgs::msg::Imu>(
            STATE_IMU, 10, std::bind(&NavigationRefactoredNode::imuStateCallback, this, std::placeholders::_1));
        ...
        //障碍物订阅
        m_Obsta_sub_ = this->create_subscription<std_msgs::msg::String>(
            STATE_OBSCTRL, 10,
            std::bind(&NavigationRefactoredNode::obstacleCallback, this, std::placeholders::_1));
        ...
        // 创建发布者
        cmd_pub_ = this->create_publisher<std_msgs::msg::String>(TASK_MOVEMENT, 10);
        ...
        navigator_.setLogCallback([this](const std::string& msg) {
            LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "%s", msg.c_str());
        });
        ...
        LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "Navigation example node started");
        LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "  - Chassis type: %s", 
            chassis_type_ == laser_navigation::ChassisType::k4WIS4WID ? "4WIS4WID" : "DiffDrive");
        LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "  - SLAM pose input: REQUIRED (for LQR tracking)");
        LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "  - Odom velocity feedback: %s", use_odom_feedback_ ? "enabled" : "disabled");
    }
...
private:
    ...
    void loadModelConfig(const std::string& path) {
        try {
            ...
            if (!file.is_open()) {
                LOG4CPLUS_WARN_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), 
            ...
            if (json.contains("motorType")) {
                ...
                if (agv_type == "4WIS4WID") {
                    ...
                    LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), 
            ...
            LOG4CPLUS_ERROR_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), 
    ...
    void loadNavigationConfig(const std::string& path) {
        try {
            ...
            nlohmann::json json = nlohmann::json::parse(file);
            LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "[navigation_config.json] Loading parameters...");
            ...
            LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "[navigation_config.json] Parameters loaded successfully");
     ...
     * @brief IMU回调（可选）
     */
    void imuStateCallback(const sensor_msgs::msg::Imu::SharedPtr msg) {
        // 目前暂时没用到IMU数据
        // 后续考虑可以根据需要提取角速度或线加速度用于改善控制平滑性
        m_Imuyaw = laser_navigation::math::MathUtils::quaternionToYaw(
            msg->orientation.x,
            msg->orientation.y,
            msg->orientation.z,
            msg->orientation.w);
    ...
    void emergedStopCallback(const std_msgs::msg::String::SharedPtr msg) {
        ...
        if (isStop) {
            ...
            {
                m_info_nav_status_.emergency_immediately_stop_ = true;
            }else if (curTask_ == "translate" || curTask_ == "rotate")
            {
                m_machine_state_ = MachineControlState::STATE_MACH_RUN_EMS_STOP;
            }
            LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "navigation pause by emerge");
            ...
            if (curTask_ == "navigation")
            {
                ...
                m_info_nav_status_.emergency_immediately_stop_ = false;
                LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "navigation reuse from emerge");
            }
            else if (curTask_ == "translate")
            {
                m_machine_state_ = MachineControlState::STATE_MACH_RUN_TRANSLATE;
                LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "translate reuse from emerge");
            }
            else if (curTask_ == "rotate")
            {
                m_machine_state_ = MachineControlState::STATE_MACH_RUN_ROLATE;
                LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "rotate reuse from emerge");
            }
        }

    }

    void obstacleCallback(std_msgs::msg::String msg) {
        
        using MotionConstraints = laser_navigation::MotionConstraints;
        ...
        try {
            ...
            if (taskType != "OBS_STATUS")
            {
                return;
            }
            obsStatus = obj["taskResult"]["status"];
        } catch (...) {
            //LOG4CPLUS_WARN_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "invalid json");
            return;
        }

        // 状态发生改变才处理
        if (obsStatus == obsStatus_)
        {
            return;
        }

        if (obsStatus_ == "EMER_STOP_REGION") // 急停区
        {
            m_info_nav_status_.obstacle_detected_ = laser_navigation::InfoNavStatus::obstacleArea::EMER_STOP_REGION;

            

            if (curTask_ == "navigation" && m_machine_state_!= MachineControlState::STATE_MACH_IDLE)
            {
                if (!m_task.constraints.empty())
                {   
                    LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "navigation pause by obs, emerge stop at %f m/s", m_task.constraints[0].obs_emergency_stop_deceleration);
                    LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("NavPos"), "navigation pause by obs, emerge stop at %f m/s\n", m_task.constraints[0].obs_emergency_stop_deceleration);
                }
                else
                {
                    LOG4CPLUS_ERROR_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "obs'size is empty");
                }

            }
            else if (curTask_ == "translate" && m_machine_state_ == MachineControlState::STATE_MACH_RUN_TRANSLATE)
            {
                
                LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "translate pause by obs, emerge stop at %f m/s",  MotionConstraints::getInstance().obs_emergency_stop_deceleration);
                LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("RotatePos"), "translate pause by obs, emerge stop at %f m/s\n",  MotionConstraints::getInstance().obs_emergency_stop_deceleration);
                
                m_machine_state_ = MachineControlState::STATE_MACH_RUN_EMERGE_STOP;
            }
            else if (curTask_ == "rotate" && m_machine_state_ == MachineControlState::STATE_MACH_RUN_ROLATE)
            {
        ...
        else if (obsStatus_ == "DEC_STOP_REGION") // 减停区
        {
            m_info_nav_status_.obstacle_detected_ = laser_navigation::InfoNavStatus::obstacleArea::DEC_STOP_REGION;
            if (curTask_ == "navigation" && m_machine_state_ != MachineControlState::STATE_MACH_IDLE)
            {
                if (!m_task.constraints.empty())
                {
                    LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "translate pause by obs, dec stop at %f m/s", m_task.constraints[0].obs_stop_deceleration);
                    LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("RotatePos"), "translate pause by obs, dec stop at %f m/s\n", m_task.constraints[0].obs_stop_deceleration);
                }
                else//使用默认值
                {
                    LOG4CPLUS_ERROR_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "obs'size is empty");
            ...
            else if (curTask_ == "translate" && m_machine_state_ == MachineControlState::STATE_MACH_RUN_TRANSLATE)
            {
                if (!m_task.constraints.empty())
                {
                    LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "translate pause by obs, dec stop at %f m/s", MotionConstraints::getInstance().obs_stop_deceleration);
                    LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("RotatePos"), "translate pause by obs, dec stop at %f m/s\n", MotionConstraints::getInstance().obs_stop_deceleration);
                }
                m_machine_state_ = MachineControlState::STATE_MACH_RUN_DEC_STOP;
            }
            else if (curTask_ == "rotate" && m_machine_state_ == MachineControlState::STATE_MACH_RUN_ROLATE)
            {
                 if (!m_task.constraints.empty())
                {
                    LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "translate pause by obs, dec stop at %f m/s",  MotionConstraints::getInstance().obs_emergency_stop_deceleration);
                    LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("RotatePos"), "translate pause by obs, dec stop at %f m/s\n",  MotionConstraints::getInstance().obs_emergency_stop_deceleration);
                }
                m_machine_state_ = MachineControlState::STATE_MACH_RUN_DEC_STOP;
            }
        } 
        else if (obsStatus_ == "NORMAL_REGION")
        { 
            //TODO 恢复急停/避障减停标志位并恢复导航
            m_info_nav_status_.obstacle_detected_ = laser_navigation::InfoNavStatus::obstacleArea::NORMAL_REGION;
            if (curTask_ == "navigation")
            {
                if (m_machine_state_ == MachineControlState::STATE_MACH_OBS_PAUSE || m_machine_state_ == MachineControlState::STATE_MACH_RUN)
                {
                    LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "navigation reuse from obsta");
                    LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("NavPos"), "navigation reuse from obsta\n");
                }
            }
            else if (curTask_ == "translate" )
            {
                if ((m_machine_state_ == MachineControlState::STATE_MACH_RUN_EMERGE_STOP ||
                    m_machine_state_ == MachineControlState::STATE_MACH_RUN_DEC_STOP ||
                    m_machine_state_ == MachineControlState::STATE_MACH_OBS_PAUSE))
                {
                    m_machine_state_ = MachineControlState::STATE_MACH_RUN_TRANSLATE;
                    LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "translate reuse from obsta");
                    LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("RotatePos"), "translate reuse from obsta\n");
                }
            }
            else if (curTask_ == "rotate")
            {
                ...
                {
                    m_machine_state_ = MachineControlState::STATE_MACH_RUN_ROLATE;
                    LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "rotate reuse from obsta");
                    LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("RotatePos"), "rotate reuse from obsta\n");
                }
            }
        }
    ...
    void goalCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
        if (!has_slam_pose_) {
            LOG4CPLUS_WARN_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "No SLAM pose received yet, ignoring goal");
            return;
        }
        ...
        if (result == 0) {
            ...
            LOG4CPLUS_ERROR_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "Failed to initialize navigation");
        }
    }
    
    void navTaskCallback(const std_msgs::msg::String::SharedPtr msg) {
        // 解析JSON格式的路径
        LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "Received path JSON: %s", msg->data.c_str());
        
        auto task = json_parser_.parseTask(msg->data);

        if (!task) {
            LOG4CPLUS_ERROR_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "Failed to parse path JSON: %s", 
        ...
        if( !m_task.codes.empty() ){
            std_msgs::msg::String msg_tmp;
            msg_tmp.data = m_task.codes;
        ...
        if( m_task.task_type == "START" ){
            // TODO 
            curTask_ = "navigation";
            handleTaskStart(m_task);

        }else if (m_task.task_type == "STOP")
        {
            handleTaskStop();
        }
        else if (m_task.task_type == "CANCEL")
        {
            handleTaskCancel();
        }
        else if (m_task.task_type == "PAUSE")
        {
            handleTaskPause();
        }
        else if (m_task.task_type == "RECOVERY")
        {
            handleTaskRecovery();
        }
        else if (m_task.task_type == "UPDATE")
        {
            handleTaskUpdate(m_task);
        }
        else if (m_task.task_type == "translate")
        {
            curTask_ = "translate";
            // TODO 考虑将平移 和 旋转 转移到excutor中
            handleTranslate(nlohmann::json::parse(msg->data));
        }
        else if (m_task.task_type == "rotate")
        {
            curTask_ = "rotate";
            handleRotate(nlohmann::json::parse(msg->data));
        }
        else
        {
            LOG4CPLUS_WARN_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "invalid task type %s", m_task.task_type.c_str());
        }  
    ...
    void controlLoop() {
        ...
        {   using NavStatus = laser_navigation::NavigationStatus;
            ...
            { // 开始转动
                if (rotateMode_ == "odom")
                {
                    nav_status = runRotate(odom_pose);
                }
                else if (rotateMode_ == "location")
                {
                    nav_status = runRotate(current_pose_);
                }
            }
            ...
            else if (m_machine_state_ == MachineControlState::STATE_MACH_PAUSE)
            {
            ...
            {
                LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "navigation finish");
            }
        }
        catch (...)
        {
            LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "throw execption");
        }
    }
    ...
    void handleTaskStart(  laser_navigation::NavigationTask& task )
    {
        ...
        if (!has_slam_pose_ || !current_pose_.valid) {
            LOG4CPLUS_WARN_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "No valid SLAM pose received yet, ignoring path");
        ...
        if (result == 0) {
            ...
            for (size_t i = 0; i < task.waypoints.size(); ++i) {
                ...
                LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "  [%zu] %s: (%.3f, %.3f, %.2f°)",
    ...
    void handleTaskStop(){
        ...
        if (curTask_ == "navigation")
        { 
            m_info_nav_status_.taskStopped_ = true;
            LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "navigation stop manul");
            
        }
        else if (curTask_ == "translate" || curTask_ == "rotate")
        {
            m_machine_state_ = MachineControlState::STATE_MACH_RUN_STOP;
            LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "translate/rotate stop manul");
    ...
    void handleTaskCancel(){
        ...
        if (curTask_ == "navigation")
        {
            m_info_nav_status_.cancel_requested_ = true;
            LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "navigation cancel manul");
            
        }
        else if (curTask_ == "translate" || curTask_ == "rotate")
        {
            LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "translate/rotate cancel manul");
     ...
     void handleTaskPause(){
        ...
        if (curTask_ == "navigation")
        {
            m_info_nav_status_.taskPaused_ = true;
        }
        else if (curTask_ == "translate" || curTask_ == "rotate")
        {
        ...
        LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "navigation pause manul");
    }

    void handleTaskRecovery(){
        if (m_machine_state_!= MachineControlState::STATE_MACH_PAUSE)//只有真停下了才考虑恢复
        {
            return;
        }
        m_machine_state_ = MachineControlState::STATE_MACH_RUN;
        m_info_nav_status_.taskResumed_ = true;

        LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "navigation recovery manul");
    ...
    void handleTaskUpdate( const laser_navigation::NavigationTask& task ){
        ...
        if ( m_machine_state_ == MachineControlState::STATE_MACH_IDLE)
        {
            LOG4CPLUS_WARN_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "navigation is not in run state");
            return;
        }
        ...
        if (is_navigating_) {
            ...
            if (result != 0) {
                LOG4CPLUS_ERROR_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "Failed to update path");
            }
    ...
    void handleTranslate(const nlohmann::json& jsonTask)
    {
        if (m_machine_state_ == MachineControlState::STATE_MACH_RUN)
        {
            LOG4CPLUS_WARN_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "navigation is running, cannot translate");
            return;
        }

        double distance, linear;
        try {
            if (!jsonTask.contains("taskParameters"))
            {
                LOG4CPLUS_WARN_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "invalid json, miss taskParameters");
                return;
            }

            if (!jsonTask["taskParameters"].contains("distance"))
            {
                LOG4CPLUS_WARN_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "invalid json, miss distance");
                return;
            }
            distance = jsonTask["taskParameters"]["distance"];

            if (!jsonTask["taskParameters"].contains("linear"))
            {
                LOG4CPLUS_WARN_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "invalid json, miss linear");
                return;
            }
            linear = jsonTask["taskParameters"]["linear"];

            if (!jsonTask["taskParameters"].contains("mode"))
            {
                LOG4CPLUS_WARN_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "invalid json, miss mode");
                return;
            }
            rotateMode_ = jsonTask["taskParameters"]["mode"];

        } catch (...) {
            // 走到这里的都是字段类型不对，比如：float当成string下发
            LOG4CPLUS_ERROR_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "invalid json, syntax error, value type is incorrect");
            return;
        }

        curTranslateTask_.acc = laser_navigation::MotionConstraints::getInstance().max_acceleration;
        curTranslateTask_.dec = laser_navigation::MotionConstraints::getInstance().max_deceleration;
        curTranslateTask_.linear = linear;
        curTranslateTask_.distance = distance;
        curTranslateTask_.timer_interval = 25;
        rotate_.setTranslateTask(curTranslateTask_);
        m_machine_state_= MachineControlState::STATE_MACH_RUN_TRANSLATE;
        LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("RotatePos"), "translate %f %f %f %f %f\n",
           curTranslateTask_.acc, curTranslateTask_.dec, curTranslateTask_.linear, curTranslateTask_.distance, curTranslateTask_.timer_interval);
    }

    void handleRotate(const nlohmann::json& jsonTask)
    {
        if (m_machine_state_ == MachineControlState::STATE_MACH_RUN)
        {
            LOG4CPLUS_WARN_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "navigation is running, cannot rotate");
            return;
        }
        ...
        try {
            ...
            if (!jsonTask["taskParameters"].contains("theta"))
            {
                LOG4CPLUS_WARN_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "invalid json, miss taskParameters");
                return;
            }
            theta = jsonTask["taskParameters"]["theta"];
            if (!jsonTask["taskParameters"].contains("angular"))
            {
                LOG4CPLUS_WARN_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "invalid json, miss taskParameters");
                return;
            }
            angular = jsonTask["taskParameters"]["angular"];
        ...
        curRotateTask_.acc = laser_navigation::MotionConstraints::getInstance().max_angular_acceleration;
        curRotateTask_.dec = laser_navigation::MotionConstraints::getInstance().max_angular_deceleration;
        curRotateTask_.angular = angular / 180.0 * M_PI;
        curRotateTask_.theta = theta / 180.0 * M_PI;
        curRotateTask_.timer_interval = 25;
        rotate_.setRotateTask(curRotateTask_);
        m_machine_state_ = MachineControlState::STATE_MACH_RUN_ROLATE;
       LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("RotatePos"), "rotate %f %f %f %f %f\n",
           curRotateTask_.acc, curRotateTask_.dec, curRotateTask_.angular, curRotateTask_.theta, curRotateTask_.timer_interval);
    }
    ...
    laser_navigation::NavigationOutput runNavigation(){
        ...
        if (!has_slam_pose_ || !current_pose_.valid) {
            LOG4CPLUS_WARN_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), 
                "Stop to Waiting for valid SLAM pose...");
            pubMovement(0.0, 0.0, 0.0);
            return laser_navigation::NavigationOutput();
        }
        ...
        //TODO 可以考虑增加一个位姿的滤波器来防止位姿跳变对导航的影响
        ...
        // TODO 要考虑处理 m_info_nav_status_ 对应的状态后，该如何变化m_info_nav_status_ 的值，不能一直处于pause或stop状态，需要改导航后考虑如何操作
        pubMovement(output.velocity.linear_x, output.velocity.linear_y, output.velocity.angular);
        
        // 发布结构化 JSON 状态（兼容 README 中 nav_status 格式）
        
        // 检查是否完成
        if (output.status == laser_navigation::NavigationStatus::kGoalReached) {
            LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "Navigation completed!");
            ...
            LOG4CPLUS_ERROR_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "Navigation error!");
            is_navigating_ = false;
    ...
    laser_navigation::NavigationStatus runTranslate(const laser_navigation::Pose2D& pose)
    {
        using NavStatus = laser_navigation::NavigationStatus;
        using MotionConstraints = laser_navigation::MotionConstraints;
        NavStatus status;

        rotate::Pose2dtr roteatePose;
        roteatePose.x = pose.x;
        roteatePose.y = pose.y;
        roteatePose.yaw = pose.yaw;
        rotate::Result result;
        try
        {
            result = rotate_.getSpeed(roteatePose);
        }
        catch (...)
        {
            LOG4CPLUS_ERROR_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "translate throw execption");
            result.linear = 0;
            result.angular = 0;
            result.status = rotate::TaskStatus::error;
        }
        LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("RotatePos"), "%s %f %f %f %f %f %d\n", pose.mode.c_str(), pose.x, pose.y, pose.yaw, result.linear, result.angular, result.status);

        if (abs(result.linear) > MotionConstraints::getInstance().max_velocity)
        {
            LOG4CPLUS_ERROR_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "linear is too big, linear: %f", result.linear);
            result.linear = MotionConstraints::getInstance().max_velocity;
        }
        if (abs(result.angular) > MotionConstraints::getInstance().max_angular_velocity)
        {
            LOG4CPLUS_ERROR_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "angle is too big, angle: %f", result.angular);
            result.angular = MotionConstraints::getInstance().max_angular_velocity;
        }

        // 出现nan数据则任务导航失败
        if ((std::isnan(result.angular) || std::isinf(result.angular)) ||
            (std::isnan(result.linear) || std::isinf(result.linear))) {
            LOG4CPLUS_ERROR_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "linear or angle is nan");
            result.linear = 0.0;
            result.angular = 0.0;
            result.status = rotate::TaskStatus::error;
        }

        if (result.status == rotate::TaskStatus::finish)
        {
            m_machine_state_ = MachineControlState::STATE_MACH_IDLE;
            status = NavStatus::kGoalReached;
        }
        else if (result.status == rotate::TaskStatus::error)
        {
            m_machine_state_ = MachineControlState::STATE_MACH_IDLE;
            status = NavStatus::kError;
        }
        else
        {
            if (curTranslateTask_.linear < 0)
            {
                status = NavStatus::kReversing;
            }
            else
            {
                status = NavStatus::kRunning;
            }
        }
        pubMovement(result.linear, 0.0,result.angular);

        return status;
    }

    laser_navigation::NavigationStatus runRotate(const laser_navigation::Pose2D& pose)
    {
        using NavStatus = laser_navigation::NavigationStatus;
        using MotionConstraints = laser_navigation::MotionConstraints;
        NavStatus status;
        ...
        {
            LOG4CPLUS_ERROR_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "rotate throw execption");
            result.linear = 0;
            result.angular = 0;
            result.status = rotate::TaskStatus::error;
        }
    ...
    laser_navigation::NavigationStatus runDecStopByObs()
    {
    
        using NavStatus = laser_navigation::NavigationStatus;
        using MotionConstraints = laser_navigation::MotionConstraints;
        NavStatus status = m_navigation_status_;

        float lineDec;
        float rotDec;
        // if (curTask_ == "navigation")
        // {
        //     if (m_navigation_status_ == NavStatus::kRotating)
        //     {
        //         if (!m_task.constraints.empty())
        //         {
        //             rotDec = m_task.constraints[0].obs_stop_deceleration;
        //         }
        //         else
        //         {
        //             LOG4CPLUS_ERROR_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "obs'size is empty");
        //             rotDec = MotionConstraints::getInstance().obs_stop_deceleration;
        //         }
        //         lineDec = 0.0;
        //     }
        //     else
        ...
        //         if (!m_task.constraints.empty())
        //         {
        //             lineDec = m_task.constraints[0].obs_stop_deceleration;
        //         }
        //         else
        //         {
        //             LOG4CPLUS_ERROR_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "obs'size is empty");
        //             lineDec = MotionConstraints::getInstance().obs_stop_deceleration;
        //         }
        //         rotDec = 0.0;
        //     }
        // }
        //else if (curTask_ == "translate")
        if (curTask_ == "translate")
        {
            lineDec = MotionConstraints::getInstance().obs_stop_deceleration;
            rotDec = 0.0;
        }
        ...
        NavStatus ret = decStop("DecByObs", lineDec, rotDec);
        if (ret == NavStatus::kGoalReached)
        {
            status = NavStatus::kObstaclePaused;
            m_machine_state_ = MachineControlState::STATE_MACH_OBS_PAUSE;
        }
    ...
    laser_navigation::NavigationStatus runEmergencyStopByObs()
    {
        using NavStatus = laser_navigation::NavigationStatus;
        using MotionConstraints = laser_navigation::MotionConstraints;
        NavStatus status = m_navigation_status_;
        ...
        //         if (!m_task.constraints.empty())
        //         {
        //             rotDec = m_task.constraints[0].obs_emergency_stop_deceleration;
        //         }
        //         else
        //         {
        //             LOG4CPLUS_ERROR_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "obs'size is empty");
        //             rotDec = MotionConstraints::getInstance().obs_emergency_stop_deceleration;
        //         }
        //         lineDec = 0.0;
        //     }
        //     else
        ...
        //         if (!m_task.constraints.empty())
        //         {
        //             lineDec = m_task.constraints[0].obs_emergency_stop_deceleration;
        //         }
        //         else
        //         {
        //             LOG4CPLUS_ERROR_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "obs'size is empty");
        //             lineDec = MotionConstraints::getInstance().obs_emergency_stop_deceleration;
        //         }
        //         rotDec = 0.0;
        //     }
        // }
    ...
    laser_navigation::NavigationStatus runEmergencyStopByEms()
    {
        static const float EMS_STOP_DEC = 10.0;
        using NavStatus = laser_navigation::NavigationStatus;
        ...
        // if (curTask_ == "navigation")
        // {
        //     if (m_navigation_status_ == NavStatus::kRunning)
        //     {
        //         rotDec = EMS_STOP_DEC;
        //         lineDec = 0.0;
        //     }
        ...
        NavStatus ret = decStop("DecByEms", lineDec, rotDec);
        if (ret == NavStatus::kGoalReached)
        {
            status = NavStatus::kEmergencyStopped;
            m_machine_state_ = MachineControlState::STATE_MACH_EMS_PAUSE;
        }

        return status;
    }

    laser_navigation::NavigationStatus runDecStopByManualPause()
    {
        using NavStatus = laser_navigation::NavigationStatus;
        using MotionConstraints = laser_navigation::MotionConstraints;
        NavStatus status = m_navigation_status_;
        ...
        if (!m_task.constraints.empty())
        {
            lineDec = m_task.constraints[0].max_deceleration;
            rotDec = 0.0;
        }
        else
        {
            LOG4CPLUS_ERROR_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "lineAttr'size is empty");
            lineDec = MotionConstraints::getInstance().max_deceleration;
            rotDec = 0.0;
        }
        NavStatus ret = decStop("DecByPause", lineDec, rotDec);
        if (ret == NavStatus::kGoalReached)
        {
            status = NavStatus::kPaused;
            m_machine_state_ = MachineControlState::STATE_MACH_PAUSE;
        }

        return status;
    }

    laser_navigation::NavigationStatus runDecStopByManualStop()
    {
        using NavStatus = laser_navigation::NavigationStatus;
        using MotionConstraints = laser_navigation::MotionConstraints;
        NavStatus status = m_navigation_status_;
        ...
        {
            lineDec = 0.0;
            rotDec = MotionConstraints::getInstance().max_deceleration;
        }

        NavStatus ret = decStop("DecByStop", lineDec, rotDec);
        if (ret == NavStatus::kGoalReached)
        {
            status = NavStatus::kIdle;
            m_machine_state_ = MachineControlState::STATE_MACH_IDLE;
            // 在停止完成后清除
            m_task.clearTask();
        }  
    ...
    void pubNavStatus()
    {
        ...
        try {
            ...
            json_obj["taskId"] = m_task.task_id;
            json_obj["taskUpdateId"] = m_task.task_update_id;
            ...
            // 走到这里的都是字段类型不对，比如：float当成string下发
            LOG4CPLUS_ERROR_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "construct nav status json failed");
            return;
        }
        ...
        if (!status.empty()) {
            ...
            LOG4CPLUS_DEBUG_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "Published nav status: %s", status.c_str());
        }   
    }
    /**
     * @brief 发布导航到达某个站点的状态
    ...
    void pubNavStation(const std::string& current, const std::string& target)
    {
        try
        {
            ...
            status_msg.data = status;
            status_pub_->publish(status_msg);
            LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "Published nav station: %s", status.c_str());
            
        }
        catch (...)
        {
            // 走到这里的都是字段类型不对，比如：float当成string下发
            LOG4CPLUS_ERROR_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "construct nav station json failed");
            return;
        }
    }
    ...
    void printNodesInfo(const std::string& prex, const laser_navigation::NavigationTask& task)
    {
        ...
        LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "%s", ss.str().c_str());
        ...
        LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("NavPos"), "%s\n", sss.str().c_str());
        LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "%s", sss.str().c_str());
        ...
        ssss 
            ...
            ;
        LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("NavPos"), "%s\n", ssss.str().c_str());
        LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "%s", ssss.str().c_str());
    }

    void pubMovement(double linear_x,double linear_y, double angular)
    {
        try
        {
            // 电机接收的是 弧度每秒
            ...
            {
            case laser_navigation::ChassisType::kDifferential:
                obj["taskType"] = "movement";
                obj["taskParameters"]["type"] = "auto";
                obj["taskParameters"]["linear"] = linear_x;
                obj["taskParameters"]["angular"] = angular;
                break;
            case laser_navigation::ChassisType::k4WIS4WID:
                obj["taskType"] = "movement";
                obj["taskParameters"]["type"] = "auto";
                obj["taskParameters"]["linearX"] = linear_x;
                obj["taskParameters"]["linearY"] = linear_y;
                obj["taskParameters"]["angular"] = angular;
                break;
    ...
    laser_navigation::NavigationStatus decStop(const std::string& tag, float lineDec, float rotDec)
    {
        std::pair<laser_navigation::NavigationStatus, laser_navigation::Velocity> ttwist;
        // 该函数暂定不处理导航时的速度，非导航时目前只有按定位做平移和旋转，只会用到下面的{rotate}模块
        // if(tag != "DecByStop")
        // {
        //     if (has_odom_velocity_)
        //     {
        //         ttwist = navigator_.decelerateStop(lineDec, rotDec, odom_vel);
        //     }
        //     else
        //     {
        //         ttwist = {laser_navigation::NavigationStatus::kGoalReached, {0.0, 0.0, 0.0}};
        //         LOG4CPLUS_WARN_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "no odom velocity, cannot dec stop properly");
        //     }
        // }
        // else
        {
            rotate::Result calcu_result = rotate_.getSpeed(lineDec,rotDec);
            ttwist.second.angular = calcu_result.angular;
            ttwist.second.linear = calcu_result.linear;
            if(calcu_result.status != rotate::TaskStatus::finish)
            {
                ttwist.first = laser_navigation::NavigationStatus::kRunning;
            }
            else
            {
                ttwist.first = laser_navigation::NavigationStatus::kGoalReached;
            }
        }
        
        LOG4CPLUS_INFO_FMT(log4cplus::Logger::getInstance("NavPos"), "%s %f %f %f %f %f %f %f %d\n", tag.c_str(), lineDec, rotDec, odom_vel.linear_x, odom_vel.linear_y,odom_vel.angular, ttwist.second.linear, ttwist.second.angular, static_cast<int>(ttwist.first));
        if (abs(ttwist.second.linear) > 1.5) {
            LOG4CPLUS_ERROR_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "linear is too big, linear: %f", ttwist.second.linear);
            ttwist.second.linear = 1;
        }
        if (abs(ttwist.second.angular) > 1.5) {
            LOG4CPLUS_ERROR_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "angle is too big, angle: %f", ttwist.second.angular);
            ttwist.second.angular = 1;
        }
        if ((std::isnan(ttwist.second.angular) || std::isinf(ttwist.second.angular)) ||
            (std::isnan(ttwist.second.linear) || std::isinf(ttwist.second.linear))) {
            LOG4CPLUS_ERROR_FMT(log4cplus::Logger::getInstance("LaserNavRefactored"), "linear or angle is nan");
            ttwist.second.linear = 0;
            ttwist.second.angular = 0;
        }

        pubMovement(ttwist.second.linear_x,ttwist.second.linear_y, ttwist.second.angular);
    ...
    void initLog()
    {
        ...
        try {
            // 根据配置文件自动初始化日志
            std::string file_path = common::Env::getInstance()->logPath() + "laser_nav.properties";
...
private:
    ...
    //速度规划器
    rotate::TranslateRotate rotate_;
    ...
    laser_navigation::Pose2D odom_pose;           // 里程计位姿（导航可选）
    laser_navigation::Velocity odom_vel;          // 里程计速度（导航可选）
    ...
    //导航避障
   // bool m_obstacle_detected_{false};
    //double m_obstacle_deceleration_{0.5};

    //取消导航
    //bool m_cancel_requested_{false};

    //导航急停
    //bool m_emergency_immediately_stop_{false};

    laser_navigation::InfoNavStatus m_info_nav_status_;

    laser_navigation::NavigationTask m_task;

    std::string curTask_;
    std::string rotateMode_{"odom"};
    rotate::TranslateTask curTranslateTask_;
    rotate::RotateTask curRotateTask_;
    // 停障状态
    std::string obsStatus_{"NORMAL_REGION"};
    ...
    rclcpp::Subscription<std_msgs::msg::String>::SharedPtr m_Obsta_sub_;
    rclcpp::Subscription<sensor_msgs::msg::Imu>::SharedPtr m_Imu_sub_;
